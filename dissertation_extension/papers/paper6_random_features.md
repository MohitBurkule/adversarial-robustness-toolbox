...
**Three caveats that bound the claim.** Three features of this control deserve explicit statement, because each narrows what §4.9 proves.

1. *The label is the trained CNN's PGD outcome.* Every AUROC in Table 6 scores a frozen-feature margin against the *trained reference CNN's* per-sample PGD vulnerability (ASR=0.96, eps=15/255). Strictly, then, we are showing that an architecture-free projection of the raw input predicts *the deployed model's* vulnerability — not that the random net is itself adversarially robust or vulnerable in the same way. This is the practically useful direction (cheap input-geometry score for a model you will actually ship), but it is a correlation-with-the-trained-model result, not a statement about the random net's own robustness. We do not claim the latter.

2. *The readout is supervised.* Each frozen feature map is followed by a trained logistic readout, so "input geometry alone" means "linearly decodable from a fixed random embedding of the input," not "label-free." A genuinely label-free baseline — k-NN distance or distance-to-class-mean in pixel space — would isolate geometry from any fitted decision rule; we did not run it, and flag it as the cleanest remaining control. That said, the raw-pixel logistic (0.8296) is the weakest supervised readout and still trails the random projection, so the *gain* from a random embedding over raw pixels is not merely label leakage.

3. *Epsilon and the three "trained-CNN" numbers.* This control is run at eps=15/255 (matching H175's saturated setting), whereas §4.1–§4.5 use eps=8/255, 20-step PGD. The trained-CNN margin AUROC therefore appears as 0.9381 (§4.1, 8/255), 0.9432 (§4.9, 15/255, PGD-10), and 0.972 (§4.2/H175, 15/255 saturated, closed-form head). These are *not* inconsistent — they are the same quantity measured at different epsilon, step count, and readout — but they are not interchangeable, and only the within-table comparisons (all at one setting) should be read as exact.
...
[14] C.-J. Simon-Gabriel, Y. Ollivier, L. Bottou, B. Schölkopf, and D. Lopez-Paz, "First-order adversarial vulnerability of neural networks and input dimension," in *Proc. ICML*, 2019.
