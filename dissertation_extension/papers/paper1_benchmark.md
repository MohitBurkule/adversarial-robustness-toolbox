...
**Per-sample vulnerability predictors.** Prediction margin and softmax confidence are natural candidates for vulnerability prediction, as low-confidence predictions place a sample near the decision boundary. Zhang et al. [8] (TRADES) formalised the margin as the central quantity in adversarial training. Several works have proposed input-space gradient norms as vulnerability indicators, motivated by the observation that large gradients correspond to rapidly varying decision boundaries [6]. Smilkov et al. [5] introduced SmoothGrad to reduce gradient noise, and Sundararajan et al. [6] proposed Integrated Gradients as a theoretically principled attribution method. Most directly related to our central finding, Łukasik et al. [14] (*Margin Consistency*) independently establish the logit margin as a per-sample non-robust-sample score and study when the margin in the model's representation space remains a faithful proxy for input-space robustness. Our contribution is complementary and empirical: a controlled head-to-head benchmark that places the margin against 50+ alternatives under matched-ASR, AutoAttack-grade, and bootstrap-CI controls, and shows that the boundary-proximity predictors collapse to a single statistically indistinguishable axis. Simon-Gabriel et al. [15] further show that adversarial vulnerability scales with input dimension, which contextualises why a single boundary-distance quantity dominates across datasets of differing intrinsic dimension.
...
**Caveat — `min_eps` is a near-oracle, not a deployable predictor.** We report `min_eps` (the smallest L-inf budget at which the FGSM-direction attack flips the sample) for completeness, but it should not be read as "the best feature." `min_eps` is by construction the per-sample distance-to-misclassification — essentially the vulnerability *label* measured at finer resolution rather than an independent signal computed ahead of the attack. Its small AUROC edge is therefore expected and uninformative: it leaks the quantity it predicts. The deployable predictors are those computable from a single clean forward/backward pass — margin (0.9575) and input gradient norm (0.9591) — and these are statistically tied with one another. We accordingly base the "no learned head-room" conclusion on the fact that the GBM does not beat these *non-oracle* features either, and treat `min_eps` only as a soft upper bound on attainable single-pass AUROC.
...
[14] A. Łukasik, et al., "Margin Consistency: visualizing and characterizing the per-sample margin as a non-robust-sample score," *arXiv:2406.18451*, 2024.

[15] C.-J. Simon-Gabriel, Y. Ollivier, L. Bottou, B. Schölkopf, and D. Lopez-Paz, "First-order adversarial vulnerability of neural networks and input dimension," in *Proc. ICML*, 2019.
