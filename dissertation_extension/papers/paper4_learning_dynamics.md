...
A widely-held intuition in adversarial machine learning is that inputs difficult to learn — those requiring more training, susceptible to forgetting, or inconsistently classified across training runs — should also be more adversarially vulnerable. We test this hypothesis directly by evaluating forgetting events [3], C-score [4], and a memorization proxy against adversarial vulnerability labels on Fashion-MNIST and related datasets. Forgetting events achieve AUROC = 0.54 for FGSM vulnerability prediction — statistically indistinguishable from random. C-score achieves AUROC = 0.53. However, the memorization proxy achieves AUROC = 0.9427 ± 0.006 with high stability. We additionally test the Area Under the Margin (AUM) [11], a *margin*-trajectory signal, and find AUROC = 0.896 — far from chance, unlike the accuracy-trajectory signals — yet it adds no information once the final margin is known (5-fold incremental ΔAUROC = −0.003, well within the ±0.037 CV standard error; Spearman 0.98 with final margin). Our title claim is scoped precisely: *accuracy-trajectory* training-dynamics signals (forgetting events, C-score) are orthogonal to adversarial vulnerability; *margin-trajectory* signals (AUM) inherit the margin's predictive power but are redundant with the final snapshot. The AUM we measure is computed on the evaluation set across epochs (not the training-set quantity of Pleiss et al. [11]), which explains its high correlation (0.983) with the final eval margin — it is a temporally-smoothed version of the same end-point measurement. These findings clarify when training-dynamics signals encode adversarial vulnerability and when they do not.
...
AUM is emphatically **not** at chance (0.896), unlike forgetting (0.560) — a result that could be read as contradicting our thesis. It does not. AUM correlates with the final margin at Spearman 0.983: it is a smoothed estimate of the same boundary-distance quantity, averaged over the last epochs of training. The decisive test is incremental signal. A 5-fold cross-validated logistic regression on [final_margin] scores CV-AUROC 0.9168 ± 0.0366; adding AUM gives 0.9135 ± 0.0359 — a change of **−0.0033** (within 0.1× the CV standard error of ≈0.037; statistically indistinguishable from zero independent information).

**Eval-set AUM vs Pleiss AUM.** A methodological note: our AUM is computed from per-epoch *evaluation-set* margins over 15 epochs, whereas Pleiss et al. [11] define AUM over *training-set* margins (to identify mislabelled training examples). These are different quantities. Our eval-AUM measures how consistently the model generalises across the course of training, which is why it correlates at Spearman 0.983 with the final eval margin — it is a temporal aggregate of the same endpoint, not an independent training-dynamics signal. A training-set AUM might differ, but it cannot be computed without access to training checkpoints per sample, so we defer that comparison to future work. The key result (no incremental information over the final margin snapshot) is robust to this distinction since any training-set AUM that correlates as highly with the final eval margin would also be redundant.
