"""
Hypothesis H100: C-Score consistency (Jiang 2021) across K=5 models predicts adversarial vulnerability.

Train K=5 CNN models with seeds 0, 1, 2, 3, 4 on Fashion-MNIST for 10 epochs.
For each test sample, compute the C-Score: fraction of models that predict it correctly.
Use Model 0 as the victim model.
Features:
  - c_score: fraction of the K=5 models predicting correctly on the sample
...
def main():
    ...
    # Evaluate all models on test set to compute C-score
    print("Evaluating models for C-score...")
    correctness = torch.zeros(N, K_MODELS, dtype=torch.float32, device=DEVICE)
    with torch.no_grad():
        for k, m in enumerate(models):
            for i in range(0, N, 512):
                logits = m(test_x[i:i+512])
                correctness[i:i+512, k] = (logits.argmax(dim=1) == test_y[i:i+512]).float()

    c_scores = correctness.mean(dim=1) # (N,)

    # Filter to correctly predicted test samples for the victim (model 0)
    correct_victim = correctness[:, 0].bool()
    x_c = test_x[correct_victim]
    y_c = test_y[correct_victim]
    c_scores_c = c_scores[correct_victim].cpu().numpy()
    N_c = x_c.size(0)
