"""
Hypothesis H144: SAT (Smooth Adversarial Training) curriculum loss smoothing on Fashion-MNIST.

Train two victims on Fashion-MNIST for 10 epochs each:
  1. Vanilla CNN (trained on clean images)
  2. SAT CNN (trained using adversarial training where the adversarial loss is computed
     at temperature T scheduled from 5.0 to 1.0 over epochs: loss = CE(logits / T, y)).
...
Analysis:
  - Compare accuracy and robustness of Vanilla vs SAT.
...
# ---- Attack Functions for Training and Evaluation ----
...
def train_sat(train_set, seed=0):
    ...
    for epoch in range(EPOCHS):
        # Schedule temperature T from 5.0 down to 1.0
        T = 5.0 - 4.0 * (epoch / float(EPOCHS - 1)) if EPOCHS > 1 else 1.0
        model.train()
        for x, y in train_loader:
            x, y = x.to(DEVICE), y.to(DEVICE)

            # Generate adversarial examples using a fast 2-step PGD during training
            x_adv = pgd_attack(model, x, y, eps=EPS, alpha=5.0/255.0, steps=2, random_start=True)

            model.train()
            optimizer.zero_grad()
            # SAT loss: cross-entropy at temperature T
            loss = F.cross_entropy(model(x_adv) / T, y)
            loss.backward()
            optimizer.step()
    return model
...
def main():
    print("=" * 60)
    print("Hypothesis H144: SAT (Curriculum Loss Smoothing)")
    print("=" * 60)
    ...
    # Train Model 2 (SAT)
    print("\nTraining SAT CNN victim...")
    t0 = time.time()
    sat_model = train_sat(train_set)
    print(f"SAT CNN trained in {time.time() - t0:.1f}s")

    # Evaluate both
    evaluate_victim(vanilla_model, test_x, test_y, "Vanilla CNN")
    evaluate_victim(sat_model, test_x, test_y, "SAT CNN")
