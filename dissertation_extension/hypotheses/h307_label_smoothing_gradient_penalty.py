"""H307: Label smoothing + input gradient penalty combination."""
...
def train_combined(meta, Xtr, Ytr, label_smooth, grad_lam):
    C.set_seed(SEED)
...
def main():
    ...
    conditions = [
        ("baseline",                  0.0,  0.0),
        ("label_smooth_only",         0.1,  0.0),
        ("grad_penalty_only",         0.0,  0.01),
        ("label_smooth_grad_penalty", 0.1,  0.01),
    ]
    results = []
    for name, ls, gl in conditions:
        t0 = time.time()
        model = train_combined(meta, Xtr, Ytr, ls, gl)
        t1 = time.time()
    ...
    out = "results/fashion_mnist/h307_label_smoothing_gradient_penalty_output.txt"
    with open(out, "w") as f:
        f.write("H307: Label Smoothing + Gradient Penalty\n" + "="*60 + "\n")
        for r in results:
