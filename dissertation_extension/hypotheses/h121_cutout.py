"""
Hypothesis H121: Cutout Defense vs Vanilla CNN on Fashion-MNIST.

Trains two CNN models matching diagnostic_test.py:
1. Vanilla model
2. Cutout model (DeVries et al., 2017) with random 8x8 patches zeroed out during training.
...
def apply_cutout(x, length=8):
    """Apply cutout augmentation: zero out random 8x8 patches in each image."""
    B, C, H, W = x.size()
    x_cut = x.clone()
    for i in range(B):
        ...
        y1 = np.clip(y_center - length // 2, 0, H)
        y2 = np.clip(y_center + length // 2, 0, H)
        x1 = np.clip(x_center - length // 2, 0, W)
        x2 = np.clip(x_center + length // 2, 0, W)
        
        x_cut[i, :, y1:y2, x1:x2] = 0.0
    return x_cut


def train_model(train_set, mode='vanilla', seed=0):
    """Train CNN model, optionally applying Cutout augmentation."""
...
def main():
    print("=" * 70)
    print("Hypothesis H121: Cutout Defense vs Vanilla CNN")
    print("=" * 70)
    ...
    print("\nTraining Cutout model...")
    model_cutout = train_model(train_set, mode='cutout', seed=0)
    ...
    # Evaluate Cutout
    print("\nEvaluating Cutout model...")
    model_cutout.eval()
    with torch.no_grad():
        correct_c = (model_cutout(x_test).argmax(dim=1) == y_test)
    x_c, y_c = x_test[correct_c], y_test[correct_c]
    feats_c, fgsm_c, pgd_c, eps_c = compute_features_and_targets(model_cutout, x_c, y_c)
    ...
    print("\nUnivariate AUROC vs PGD Flip (Cutout):")
    auroc_pgd_c = evaluate_auroc(feats_c, pgd_c, feature_names)
    for name, score in auroc_pgd_c.items():
        print(f"  {name:<15} : {score:.4f}")
        
    print("\n" + "=" * 50)
    print("SUMMARY OF AUROC CHANGES (Cutout vs Vanilla PGD)")
    print("=" * 50)
    for name in feature_names:
        diff = auroc_pgd_c[name] - auroc_pgd_v[name]
        print(f"  {name:<15} : Cutout={auroc_pgd_c[name]:.4f}  Vanilla={auroc_pgd_v[name]:.4f}  Change={diff:+.4f}")
