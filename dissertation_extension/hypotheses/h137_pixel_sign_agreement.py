"""
Hypothesis H137: Per-pixel gradient sign agreement across K=5 CNN models predicts adversarial vulnerability.

Train K=5 CNN models with different seeds (0..4) on Fashion-MNIST for 10 epochs each.
For each correctly classified test sample, compute the input gradient w.r.t. cross-entropy loss for each model.
Compute the sign of the input gradient for each model: sign(grad) in {-1, 1}.
Compute the pixel-level agreement: fraction of the 784 pixels where all K=5 models agree on the gradient sign.
Use Model 0 as the victim model.

Features:
  - pixel_sign_agreement: fraction of pixels where all K models' gradient signs agree
...
def get_input_grad_sign(model, x, y):
    """Compute the sign of input gradient for each sample in a batch."""
...
def main():
    ...
    # Compute gradient signs for each model in batches
    print("Computing gradient sign agreement across models...")
    agreement_list = []
    for i in range(0, N_c, 512):
        ...
        # Collect grad signs from K models: shape (B_curr, K, 1, 28, 28)
        signs = torch.zeros(B_curr, K_MODELS, 1, 28, 28, device=DEVICE)
        for k, m in enumerate(models):
            signs[:, k] = get_input_grad_sign(m, xb, yb)
            
        # Sum signs: absolute sum will be equal to K (5) if and only if all agree on the sign
        sum_signs = signs.sum(dim=1) # (B_curr, 1, 28, 28)
        agree_mask = (sum_signs.abs() == K_MODELS)
        agree_fraction = agree_mask.float().mean(dim=(1, 2, 3)) # (B_curr,)
        agreement_list.append(agree_fraction)

    pixel_sign_agreement = torch.cat(agreement_list).cpu().numpy()
    ...
    feats = {
        "pixel_sign_agreement": pixel_sign_agreement,
