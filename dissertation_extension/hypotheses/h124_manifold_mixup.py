"""
Hypothesis H124: Manifold Mixup Defense vs Vanilla CNN on Fashion-MNIST.

Trains two CNN models matching diagnostic_test.py:
1. Vanilla model
2. Manifold Mixup model (Verma et al., 2019) where mixup is applied to hidden layers.
...
class ManifoldMixupCNN(nn.Module):
    """Small CNN matching diagnostic_test.py architecture supporting Manifold Mixup."""
    ...
    def forward(self, x, mixup_layer=-1, lam=1.0, index=None):
        # Eligible layers to mix:
        # 0: input
        # 1: after c1
        # 2: after c2
        # 3: after fc1
        
        if mixup_layer == 0 and index is not None:
            x = lam * x + (1.0 - lam) * x[index]
            
        x = F.relu(self.c1(x))
        if mixup_layer == 1 and index is not None:
            x = lam * x + (1.0 - lam) * x[index]
            
        x = F.relu(self.c2(x))
        if mixup_layer == 2 and index is not None:
            x = lam * x + (1.0 - lam) * x[index]
        ...
        x = F.relu(self.fc1(x))
        if mixup_layer == 3 and index is not None:
            x = lam * x + (1.0 - lam) * x[index]
...
def train_model(train_set, mode='vanilla', seed=0, alpha_mixup=2.0):
    """Train CNN model, optionally applying Manifold Mixup."""
    torch.manual_seed(seed)
    np.random.seed(seed)
    train_loader = DataLoader(train_set, BATCH, shuffle=True, num_workers=2)
    
    model = ManifoldMixupCNN(N_CLASSES).to(DEVICE)
    ...
    for epoch in range(EPOCHS):
        ...
        for x, y in train_loader:
            ...
            if mode == 'manifold_mixup':
                # Randomly select layer: 0 (input), 1 (after c1), 2 (after c2), 3 (after fc1)
                mixup_layer = np.random.randint(0, 4)
                lam = np.random.beta(alpha_mixup, alpha_mixup)
                index = torch.randperm(x.size(0)).to(DEVICE)
                
                logits = model(x, mixup_layer=mixup_layer, lam=lam, index=index)
                loss = lam * F.cross_entropy(logits, y) + (1.0 - lam) * F.cross_entropy(logits, y[index])
            else:
                logits = model(x)
...
def main():
    print("=" * 70)
    print("Hypothesis H124: Manifold Mixup Defense vs Vanilla CNN")
    print("=" * 70)
    ...
    print("\nTraining Manifold Mixup model...")
    model_mixup = train_model(train_set, mode='manifold_mixup', seed=0, alpha_mixup=2.0)
    ...
    # Evaluate Manifold Mixup
    print("\nEvaluating Manifold Mixup model...")
    model_mixup.eval()
    with torch.no_grad():
        correct_c = (model_mixup(x_test).argmax(dim=1) == y_test)
    x_c, y_c = x_test[correct_c], y_test[correct_c]
    feats_c, fgsm_c, pgd_c, eps_c = compute_features_and_targets(model_mixup, x_c, y_c)
    ...
    print("\n" + "=" * 50)
    print("MANIFOLD MIXUP MODEL RESULTS")
    print("=" * 50)
    ...
    print("\nUnivariate AUROC vs PGD Flip (Manifold Mixup):")
    auroc_pgd_c = evaluate_auroc(feats_c, pgd_c, feature_names)
    for name, score in auroc_pgd_c.items():
        print(f"  {name:<15} : {score:.4f}")
        
    print("\n" + "=" * 50)
    print("SUMMARY OF AUROC CHANGES (Manifold Mixup vs Vanilla PGD)")
    print("=" * 50)
    for name in feature_names:
        diff = auroc_pgd_c[name] - auroc_pgd_v[name]
        print(f"  {name:<15} : Manifold Mixup={auroc_pgd_c[name]:.4f}  Vanilla={auroc_pgd_v[name]:.4f}  Change={diff:+.4f}")
