"""
Hypothesis H123: AugMix Defense vs Vanilla CNN on Fashion-MNIST.

Trains two CNN models matching diagnostic_test.py:
1. Vanilla model
2. AugMix model (Hendrycks et al., 2020) with random mixture of augmentations and Jensen-Shannon consistency loss.
...
def augment(image):
    """Apply a random single augmentation operation suitable for Fashion-MNIST (1 channel)."""
    op = np.random.choice(['rotate', 'translate', 'shear', 'solarize', 'adjust_contrast'])
    img = image.clone()
    if op == 'rotate':
        angle = np.random.uniform(-30, 30)
        img = TF.rotate(img, angle)
    elif op == 'translate':
        dx = int(np.random.uniform(-4, 4))
        dy = int(np.random.uniform(-4, 4))
        img = TF.affine(img, angle=0, translate=[dx, dy], scale=1.0, shear=0)
    elif op == 'shear':
        sh = np.random.uniform(-15, 15)
        img = TF.affine(img, angle=0, translate=[0, 0], scale=1.0, shear=[sh, 0])
    elif op == 'solarize':
        threshold = np.random.uniform(0.3, 0.7)
        img = torch.where(img < threshold, img, 1.0 - img)
...
def augmix(image, width=3, depth=-1, alpha=1.0):
    """AugMix algorithm: convexly mix width paths of random augmentations."""
    mix = torch.zeros_like(image)
    weights = np.random.dirichlet([alpha] * width)
    for i in range(width):
        img_augmented = image.clone()
        d = np.random.randint(1, 4) if depth == -1 else depth
        for _ in range(d):
            img_augmented = augment(img_augmented)
        mix += weights[i] * img_augmented
...
def train_model(train_set, mode='vanilla', seed=0, lambda_js=12.0):
    """Train CNN model, optionally applying AugMix with Jensen-Shannon loss."""
    ...
    for epoch in range(EPOCHS):
        ...
        for x, y in train_loader:
            ...
            if mode == 'augmix':
                # Generate two different AugMix trajectories for the batch
                x_aug1 = torch.stack([augmix(xi) for xi in x]).to(DEVICE)
                x_aug2 = torch.stack([augmix(xi) for xi in x]).to(DEVICE)
                ...
                loss_jsd = (
                    F.kl_div(p_clean.log(), p_mixture, reduction='batchmean') +
                    F.kl_div(p_aug1.log(), p_mixture, reduction='batchmean') +
                    F.kl_div(p_aug2.log(), p_mixture, reduction='batchmean')
                ) / 3.0
                
                loss = loss_ce + lambda_js * loss_jsd
            else:
...
def main():
    print("=" * 70)
    print("Hypothesis H123: AugMix Defense vs Vanilla CNN")
    print("=" * 70)
    ...
    print("\nTraining AugMix model...")
    model_augmix = train_model(train_set, mode='augmix', seed=0, lambda_js=12.0)
    ...
    # Evaluate AugMix
    print("\nEvaluating AugMix model...")
    model_augmix.eval()
    with torch.no_grad():
        correct_c = (model_augmix(x_test).argmax(dim=1) == y_test)
    x_c, y_c = x_test[correct_c], y_test[correct_c]
    feats_c, fgsm_c, pgd_c, eps_c = compute_features_and_targets(model_augmix, x_c, y_c)
    ...
    print("\nUnivariate AUROC vs PGD Flip (AugMix):")
    auroc_pgd_c = evaluate_auroc(feats_c, pgd_c, feature_names)
    for name, score in auroc_pgd_c.items():
        print(f"  {name:<15} : {score:.4f}")
        
    print("\n" + "=" * 50)
    print("SUMMARY OF AUROC CHANGES (AugMix vs Vanilla PGD)")
    print("=" * 50)
    for name in feature_names:
        diff = auroc_pgd_c[name] - auroc_pgd_v[name]
        print(f"  {name:<15} : AugMix={auroc_pgd_c[name]:.4f}  Vanilla={auroc_pgd_v[name]:.4f}  Change={diff:+.4f}")
