"""
Hypothesis H118: CURE (Curvature Regularization) vs Vanilla CNN on Fashion-MNIST.

Trains two CNN models matching diagnostic_test.py:
1. Vanilla model
2. CURE model (Moosavi-Dezfooli et al., CVPR 2019) with curvature regularization.
...
def get_cure_reg(model, inputs, targets, h=3.0, lambda_=4.0):
    """Compute CURE regularization term (Moosavi-Dezfooli et al., CVPR 2019)."""
    inputs = inputs.clone().detach().requires_grad_(True)
    outputs = model(inputs)
    loss = F.cross_entropy(outputs, targets)
    grad = torch.autograd.grad(loss.sum(), inputs, create_graph=True)[0]
    
    grad_flat = grad.view(inputs.size(0), -1)
    grad_norm = grad_flat.norm(2, dim=1, keepdim=True) + 1e-8
    z = (h * (grad_flat / grad_norm)).view_as(grad).detach()
    
    outputs_pos = model(inputs + z)
    loss_pos = F.cross_entropy(outputs_pos, targets)
    ...
    reg = grad_diff.view(inputs.size(0), -1).norm(2, dim=1)
    return lambda_ * reg.mean()


def train_model(train_set, mode='vanilla', seed=0):
    """Train CNN model, optionally with CURE regularization."""
    ...
    for epoch in range(EPOCHS):
        ...
        for x, y in train_loader:
            ...
            if mode == 'cure':
                # CURE requires double backprop
                loss_ce = F.cross_entropy(model(x), y)
                loss_reg = get_cure_reg(model, x, y, h=3.0, lambda_=4.0)
...
def main():
    print("=" * 70)
    print("Hypothesis H118: CURE Regularization vs Vanilla CNN")
    print("=" * 70)
    ...
    print("\nTraining CURE model...")
    model_cure = train_model(train_set, mode='cure', seed=0)
    ...
    # Evaluate CURE
    print("\nEvaluating CURE model...")
    model_cure.eval()
    with torch.no_grad():
        correct_c = (model_cure(x_test).argmax(dim=1) == y_test)
    x_c, y_c = x_test[correct_c], y_test[correct_c]
    feats_c, fgsm_c, pgd_c, eps_c = compute_features_and_targets(model_cure, x_c, y_c)
    ...
    print("\nUnivariate AUROC vs FGSM Flip (Vanilla):")
    auroc_fgsm_v = evaluate_auroc(feats_v, fgsm_v, feature_names)
    for name, score in auroc_fgsm_v.items():
    ...
    print("\nUnivariate AUROC vs FGSM Flip (CURE):")
    auroc_fgsm_c = evaluate_auroc(feats_c, fgsm_c, feature_names)
    for name, score in auroc_fgsm_c.items():
        print(f"  {name:<15} : {score:.4f}")
        
    print("\nUnivariate AUROC vs PGD Flip (CURE):")
    auroc_pgd_c = evaluate_auroc(feats_c, pgd_c, feature_names)
    for name, score in auroc_pgd_c.items():
        print(f"  {name:<15} : {score:.4f}")
        
    print("\n" + "=" * 50)
    print("SUMMARY OF AUROC CHANGES (CURE vs Vanilla PGD)")
    print("=" * 50)
    for name in feature_names:
        diff = auroc_pgd_c[name] - auroc_pgd_v[name]
        print(f"  {name:<15} : CURE={auroc_pgd_c[name]:.4f}  Vanilla={auroc_pgd_v[name]:.4f}  Change={diff:+.4f}")
