#!/bin/bash
# run_patch_benchmark.sh
# Automated runner to execute hypotheses back-to-back on CIFAR-10 or Imagenette and push results to GitHub.
...
# Parse dataset argument (default: cifar10)
DATASET="${1:-cifar10}"
...
export PATCH_DATASET_NAME="$DATASET_LOWER"

# Set target results directory
RESULTS_DIR="dissertation_extension/results/$DATASET_LOWER"
mkdir -p "$RESULTS_DIR"
...
for script in "${HYPOTHESES[@]}"; do
    ...
    # Check if this script has already been run to avoid redundant execution
    if [ -f "$out_file" ]; then
    ...
    # Run the patched python script using the virtual environment
